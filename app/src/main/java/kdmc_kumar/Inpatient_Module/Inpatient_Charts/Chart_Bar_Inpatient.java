...
public class Chart_Bar_Inpatient extends AppCompatActivity {
    ...
    private void GetInitialize() {
        ...
        if (this.b != null) {
            ...
            String Patient_Name = BaseConfig.GetValues("select name as ret_values from Patreg where Patid='" + this.Patient_Id + '\'');//, Chart_Bar_Inpatient.this);
            String Patient_AgeGender = BaseConfig.GetValues("select age||'-'||gender as ret_values from Patreg where Patid='" + this.Patient_Id + '\'');//, Chart_Bar_Inpatient.this);
...
//**********************************************************************************************
    ...
    private ArrayList<BarDataSet> getDataSet(String Id) {
        ...
        String Query = "select bp,bpd,pulse,temp,resp,IFNULL(spo2,'0')as spo2,Actdate from Inpatient_MainChart where patid='" + this.Patient_Id + "' order by id desc;";
    ...
    private ArrayList<String> getXAxisValues(String Id) {
        ...
        SQLiteDatabase db = BaseConfig.GetDb();//Chart_Bar_Inpatient.this);
        String Query = "select  Actdate from Inpatient_MainChart where patid='" + this.Patient_Id + "' order by id desc";
        Cursor c = db.rawQuery(Query, null);
        if (c != null) {
            if (c.moveToFirst()) {
                do {
